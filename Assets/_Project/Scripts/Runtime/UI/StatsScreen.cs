...
{
    /// <summary>
    /// Placeholder stats panel. Session 3 will replace the body with a real summary parsed
    /// from stats.json. For now it only reports whether a stats file exists.
    /// </summary>
    public class StatsScreen : MonoBehaviour
    {
        ...
        private void BuildUI(Transform parent)
        {
            ...
            string statsPath = Path.Combine(Application.persistentDataPath, StatsFileName);
            bool hasStats = File.Exists(statsPath);

            var body = UIFactory.CreateText(_root.transform,
                hasStats
                    ? "Stats display coming in session 3."
